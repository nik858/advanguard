...
// Premium variant of Hero: identical markup, but mounts PremiumOrderForm
// instead of the free OrderForm — including inside the editor, so the operator
// edits the card he actually ships.
export function PremiumHero({ hero, order, edit = false, style }: { hero: HeroContent; order: OrderContent; edit?: boolean; style?: React.CSSProperties }) {
  return (
    <section className="ac-hero" aria-labelledby="what-is-h2" style={style}>
      <div className="ac-hero__grid">
        ...
        <Reveal as="div" className="ac-order-wrap">
          <PremiumOrderForm content={order} edit={edit} />
        </Reveal>
      </div>
    </section>
  );
