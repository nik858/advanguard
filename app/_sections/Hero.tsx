...
import { MediaSpacingHandles } from "../_editor/MediaSpacingHandles";
import { HeroCopyBlocks } from "./_shared/HeroCopyBlocks";
      ...
      <div className="ac-hero__grid">
        <div className="ac-hero__copy">
          ...
          <HeroCopyBlocks
            ...
            mediaSlot={(edit || mediaUrl(hero.sectionImage) || (hero.sectionVideoUrl ?? "").trim()) ? (
              // In-copy media block. zIndex in edit mode keeps the MediaSlot
              // popover above the following text blocks (Reveal creates
              // stacking contexts that would otherwise paint over it).
              <div
                style={{
                  position: "relative",
                  marginTop: hero.sectionMediaMt ?? 18,
                  marginBottom: hero.sectionMediaMb ?? 0,
                  zIndex: edit ? 30 : undefined,
                }}
              >
                {edit && (
                  <MediaSpacingHandles
                    mtPath="hero.sectionMediaMt"
                    mbPath="hero.sectionMediaMb"
                    currentMt={hero.sectionMediaMt}
                    currentMb={hero.sectionMediaMb}
                  />
                )}
                ...
                {(edit || (hero.sectionVideoUrl ?? "").trim()) && (
                  <Resizable path="hero.sectionVideoUrl" label="section video size">
                    <Erasable path="hero.sectionVideoUrl" label="section video">
                      <Reveal delay={140}>
                        <div className="ac-hero__video" style={{ position: "relative", marginTop: edit ? 12 : 0 }}>
